...
public class ConfigApp extends JFrame {
	...
	DataCapturePanel capturePanel_AddGesture;
	...
	ActionListener capture_alBtns = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnSaveGestureRawTrainData) {
				oprMed.saveCaptured(capturePanel_AddGesture.getCapturedRawFeature(), captureDataCMB.getSelectedItem().toString());
			}
			if (e.getSource() == btnReplay_Capture) {
				replayCaptured(bringCapturePanel());
			}
		}
	};
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			mainTabbedPane.addTab("Add/Capture Gesture", null, getCapturePanel(), null);
			mainTabbedPane.addTab("Verify", null, getVerifyPanel(), null);
			mainTabbedPane.addTab("Train the Program to recognize Gesture", null, getTrainPanel(), null);
			...
			JLabel lblMouseGestureRecognition = new JLabel("Gesture Recognition");
	...
	private JLabel getAboutLabel() {
		if (about == null) {
			about = new JLabel("Advancing a developement originally made by Ganesh Tiwari");
	...
	private JPanel getCapturePanel() {
		if (capturePanel == null) {
			...
			capturePanel.add(bringCapturePanel());

			JLabel lblNewGesture = new JLabel("Name new Gesture");
...
//			verifyPanel.add(getDCPVerify(), null);
	...
	private DataCapturePanel getDCPVerify() {
		if (dcpVerify == null) {
			...
			dcpVerify.setBackground(Color.CYAN);
		}
		return dcpVerify;
	}

	private DataCapturePanel bringCapturePanel() {
		if (capturePanel_AddGesture == null) {
			capturePanel_AddGesture = new DataCapturePanel();
			capturePanel_AddGesture.setBounds(new Rectangle(15, 15, 430, 275));
			capturePanel_AddGesture.setBackground(Color.CYAN);			
		}
		return capturePanel_AddGesture;
	}
	...
	private DataCapturePanel getDCPDataEditor() {
		if (dcpDataEditor == null) {
			...
			capturePanel_AddGesture.setBackground(Color.black);	
		}
	...
	public static void main(String[] args) {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				ConfigApp thisClass = new ConfigApp();
				thisClass.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
				thisClass.setTitle("Gesture configuration window");
