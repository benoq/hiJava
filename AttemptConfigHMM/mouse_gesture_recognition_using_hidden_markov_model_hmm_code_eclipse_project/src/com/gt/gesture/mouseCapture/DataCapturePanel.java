...
/**
 * 
 * @author Ganesh // adds by grubbauer
 * 
 */
public class DataCapturePanel extends JPanel implements MouseMotionListener, MouseListener {
	...
	private final int RadiusOfClickVisualization = 6;
	private boolean drawingStarted = false;
	private Color BackgroundColorGeneral = Color.CYAN;
	...
	public DataCapturePanel() {
		setBackground(BackgroundColorGeneral);
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < rf_curTime.size(); i++) {
			...
			((Graphics2D) g).drawOval((int) rf_drawPoint.get(i).getX(), (int) rf_drawPoint.get(i).getY(), RadiusOfClickVisualization, RadiusOfClickVisualization);
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		// started
		drawingStarted = true;
		this.setBackground(BackgroundColorGeneral);
		rf_curTime.clear();
		rf_drawPoint.clear();
